...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
   ...
   if(!b)
   {
      std::cout << "tgamma_imp called with " << type_id<T>().name_demangled() << " " << type_id<Lanczos>().name_demangled() << std::endl;
      b = true;
   }
#endif
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
   ...
   if(!b)
   {
      std::cout << "lgamma_imp called with " << type_id<T>().name_demangled() << " " << type_id<Lanczos>().name_demangled() << std::endl;
      b = true;
   }
#endif
